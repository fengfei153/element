...
var copyArray = function copyArray(arr, props) {
  ...
  var result = [];
  var configurableProps = ['__IS__FLAT__OPTIONS', 'label', 'value', 'disabled', 'itemClass'];
  ...
  computed: {
    activeOptions: {
      get: function get() {
        ...
        var configurableProps = ['label', 'value', 'children', 'disabled', 'itemClass'];
    ...
    var menus = this._l(activeOptions, function (menu, menuIndex) {
      ...
      var items = _this3._l(menu, function (item) {
        ...
        if (!item.disabled) {
          ...
          // 如果选项被禁用但是有children，则允许展开但是不可选中  by fengfei153
          var _triggerEvent = {
            click: 'click',
            hover: 'mouseenter'
          }[expandTrigger];
          var _triggerHandler = function _triggerHandler() {
            if (_this3.visible) {
              _this3.activeItem(item, menuIndex);
              _this3.$nextTick(function () {
                // adjust self and next level
          ...
          events.on[_triggerEvent] = _triggerHandler;
          events.on['mousedown'] = function () {
            _this3.clicking = true;
          };
          events.on['focus'] = function () {
      ...
      if (filteredFlatOptions.length > 0) {
        filteredFlatOptions = filteredFlatOptions.map(function (optionStack) {
          return {
            ...
            disabled: optionStack[optionStack.length - 1][_this5.disabledKey]
          };
        });
      } else {
        filteredFlatOptions = [{
